...
async function testAll() { const d = await loadBibliaLivre(); console.log(d.length, d.map(x=>x.abrev)); for(let b of d) { try { await fetchFromBibliaLivre(b.abrev, 1); } catch(e) { console.error('Error:', b.abrev, e) } } console.log('Done'); } testAll();